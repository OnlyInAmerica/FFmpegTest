...
public class LiveHLSRecorder extends HLSRecorder {
	...
	private HLSFileObserver observer; 	// Must hold reference to observer to
										// continue receiving events
	private ExecutorService uploadService;
	private int uploadServiceQueueLength;

	public static final String INTENT_ACTION = "HLS"; 	// Intent action broadcast
														// to
														// LocalBroadcastManager

	public enum HLS_STATUS {
		LIVE, COMPLETE
	};

	private boolean sentIsLiveBroadcast = false; 	// Only send
													// "broadcast is live"
													// intent once per recording
	...
	/**
	 * We'll create a single thread ExecutorService for uploading, and
	 * immediately submit the .ts and .m3u8 jobs in tick-tock fashion.
	 * Currently, the fileObserver callbacks don't return until the entire
	 * upload is complete, which means by the time the first .ts uploads, the
	 * the next callback (the .m3u8 write) is called when the underlying action
	 * has been negated by future (but uncalled) events
	 */
	@Override
	public void startRecording(final String outputDir) {
		super.startRecording(outputDir);
		temp = new File(getOutputDirectory(), "temp"); // make temp directory
														// for .m3u8s for each
														// upload state
		temp.mkdirs();
		...
		if (!UPLOAD_TO_S3)
			return;
		observer = new HLSFileObserver(getOutputDirectory().getAbsolutePath(),
				new HLSCallback() {
					...
					public void onSegmentComplete(final String path) {
						lastSegmentWritten++;
						if (VERBOSE)
							Log.i(TAG, ".ts segment written: " + path);
						uploadService.submit(new Runnable() {

							@Override
							public void run() {
								File orig = new File(path);
								String url = s3Client.upload(getUUID()
										+ File.separator + orig.getName(),
										orig, segmentUploadedCallback);
								if (VERBOSE)
									Log.i(TAG,
											".ts segment destination url received: "
													+ url);
					...
					public void onManifestUpdated(String path) {
						if (VERBOSE)
							Log.i(TAG, ".m3u8 written: " + path);
						// Copy m3u8 at this moment and queue it to uploading
						// service
						final File orig = new File(path);
						final File copy = new File(temp, orig.getName()
								.replace(".m3u8",
										"_" + lastSegmentWritten + ".m3u8"));

						if (TRACE)
							Trace.beginSection("copyM3u8");
						try {
		...
		public void onProgress(ProgressEvent progressEvent, long bytesUploaded,
			...
			if (VERBOSE)
				Log.i(TAG, String.format(
						".ts segment upload progress: %d event: %d",
						percentUploaded, progressEvent.getEventCode()));
			if (progressEvent.getEventCode() == ProgressEvent.COMPLETED_EVENT_CODE) {
				if (VERBOSE)
					Log.i(TAG, ".ts segment upload success");
			} else if (progressEvent.getEventCode() == ProgressEvent.FAILED_EVENT_CODE) {
				if (VERBOSE)
					Log.i(TAG, ".ts segment upload failed");
			}
		}
	...
	S3Callback manifestUploadedCallback = new S3Callback() {
		...
		public void onProgress(ProgressEvent progressEvent, long bytesUploaded,
				int percentUploaded) {
			if (VERBOSE)
				Log.i(TAG, String.format(".m3u8 upload progress: %d event: %d",
						percentUploaded, progressEvent.getEventCode()));
			if (progressEvent.getEventCode() == ProgressEvent.COMPLETED_EVENT_CODE) {
				if (VERBOSE)
					Log.i(TAG, ".m3u8 upload success");
			} else if (progressEvent.getEventCode() == ProgressEvent.FAILED_EVENT_CODE) {
				if (VERBOSE)
					Log.i(TAG, ".m3u8 upload failed");
			}
		}
	...
	/**
	 * Broadcasts a message to the LocalBroadcastManager indicating the HLS
	 * stream is live. This message is receivable only within the hosting
	 * application
	 * 
	 * @param url
	 *            address of the HLS stream
	 */
	...
	private void broadcastRecordingIsComplete(String url) {
		Log.d(TAG, String.format("Broadcasting Complete for HLS link: %s", url));
		Intent intent = new Intent(INTENT_ACTION);
		intent.putExtra("url", url);
		intent.putExtra("status", HLS_STATUS.COMPLETE);
		LocalBroadcastManager.getInstance(c).sendBroadcast(intent);
	}
	...
	private void finishIfUploadServiceComplete() {
		if (uploadServiceQueueLength == 0 && !isRecording()) {
			...
			if(hlsUrl != null)
				broadcastRecordingIsComplete(hlsUrl);
			if (cb != null)
